    ...
    def execute(self,now,T_Pump):
        ...
        if self.typeOp in ['HEAT','PAUS']:
            ...
            if float(valSensor1) < float(self.tempRef()): # Shake
                ...
                if menus.options['g'][3] and self.sensor1 == 'warranty':
                    State.greasy = True
...
opSequences = {
    ...
    'C': # Désinfection thermique
        [ Operation('Dett','HEAT',ref='C',dump=False,programmable=True),
          ...
          Operation('DetH','TRAK','output','input', base_speed=MAX_SPEED, min_speed=-pumpy.maximal_liters, ref='C', duration=lambda:menus.options['c'][3], shake_qty=TOTAL_VOL/2.1,dump=False)
          #Operation('DetR','REVR',ref='C',base_speed=MAX_SPEED,qty=-TOTAL_VOL,dump=False),
